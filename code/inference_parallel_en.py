...
def predict(dataloader, model, dataset, output_path, task2identifier):
    ...
    for task, preds in output_dic.items():
        dataset[task] = torch.tensor(preds).detach().cpu().numpy()
    print(dataset)
...
if __name__ == '__main__':
   ...
   #                     help='separator in the csv file')
    ...
    eval = InferenceDataset(path_to_dataset=args.testdata, tokenizer=tokenizer, text_col=args.text_col)
    dataloader = DataLoader(eval, batch_size=args.batch_size)
    model = model.cuda()
    predict(dataloader=dataloader, model=model, dataset=eval.dataset,
